...
- **下一个自然动作（O1 已落地后，2026-06-15）**：本体化沿 P1.5 推进——
  - **O4 本体合并（不卡挂起，工程活，推荐先做）**：把 `config/p4_opportunity_map.yml`（楚天/森松/东富龙档案+竞品+capex系数）、`data/products_analysis.json`（威胁 Bürkert4.3/Gemü4.0/ESG2.7）接成 O1 entities 的富属性种子，终结双本体——registry id 已对齐，"同 id 直接接档案"已铺好。
  - **O2 建图（卡头号挂起）**：补 spec 位承重边 `ESG—has-spec-position→{楚天/森松/东富龙}`，**直接撞 §5 挂起未知**（ESG 进没进这几家 BOM），需用户先确认。
  - 旁路备选：赢面消费链路（O3，需先攒处置标签）；源攻坚挪 CI；P2#6 region 字段（见下）。

...
### 4. ✅ 装备商订单簿监测（制药出海增量）— 已完成 2026-06-25（PR #23）
- `engine/sources/orderbook.py`：盯 OEM（名单从 `entities.yml` type=oem 取，单一来源）的 新签/中标/海外/扩产 披露，复用巨潮 fulltext + 订单簿意图词过滤。接进 `run.py` collect（默认）。
- `build.py`：订单簿信号常无工况关键词 → 改为「业主即在册 OEM」放行无工况事件，用 OEM 的 O4 档案兜底阀型/行业；owner→OEM→spec(O2-A) 驱动「盯订单簿/随产线复制」动作。
- 验收（--sample）：东富龙动能信号 → 装备商订单簿事件，spec=in、阀型来自 O4、win 0.9。51 测试全过（+2）。**真·cninfo 查询需网络/CI**（机制同 cninfo.py）。
